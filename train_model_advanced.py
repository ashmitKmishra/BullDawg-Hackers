    ...
    def train(self, features_df: pd.DataFrame, labels_df: pd.DataFrame, 
        ...
        for i, benefit in enumerate(BENEFIT_TYPES, 1):
            ...
            params = {
                ...
                'boosting_type': 'gbdt',
                'num_leaves': 255,  # Maximum depth (2^8 - 1)
                'max_depth': 15,  # Extremely deep
                'learning_rate': 0.01,  # Very slow learning
                'n_estimators': 2000,  # Many more trees
                'subsample': 0.95,
                'colsample_bytree': 0.95,
                'reg_alpha': 0.001,  # Almost no L1
                'reg_lambda': 0.1,  # Almost no L2
                'min_child_samples': 5,  # Allow very small leaves
                'scale_pos_weight': scale_pos_weight,
                'random_state': 42,
                'verbose': -1,
                'min_split_gain': 0.0001,  # Allow tiny gains
                ...
                'lambda_l1': 0.001,
                'lambda_l2': 0.1,
                'max_bin': 511,  # Maximum bins for best splits
                'num_threads': -1,
                'force_row_wise': True
            }
